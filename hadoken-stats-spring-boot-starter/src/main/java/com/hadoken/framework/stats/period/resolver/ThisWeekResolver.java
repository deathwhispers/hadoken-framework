 ...
 */
public class ThisWeekResolver implements PeriodResolver {
    @Override
    public <T extends StatsQueryDto> TimeRange resolve(T request, ZoneId zoneId) {
        ...
        if (firstDayOfWeek == null) {
            firstDayOfWeek = DayOfWeek.MONDAY; // 默认周一
        }
        LocalDate start = now.with(TemporalAdjusters.previousOrSame(firstDayOfWeek));
