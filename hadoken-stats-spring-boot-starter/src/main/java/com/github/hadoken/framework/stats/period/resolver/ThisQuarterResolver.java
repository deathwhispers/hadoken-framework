package com.github.hadoken.framework.stats.period.resolver;

import com.github.hadoken.framework.stats.period.adjuster.QuarterAdjusters;
import com.github.hadoken.framework.stats.period.model.StatsQueryDto;
import com.github.hadoken.framework.stats.period.model.TimeRange;
import org.springframework.stereotype.Component;

import java.time.LocalDate;
import java.time.LocalDateTime;
import java.time.LocalTime;
import java.time.ZoneId;

/**
 * @author yanggj
 * @version 1.0.0
 * Created on 2025/8/11 10:58
 */
@Component
public class ThisQuarterResolver implements PeriodResolver {

    @Override
    public <T extends StatsQueryDto> TimeRange resolve(T request, ZoneId zoneId) {
        LocalDate now = LocalDate.now(zoneId);
        LocalDate start = now.with(QuarterAdjusters.firstDayOfQuarter());
        LocalDate end = now.with(QuarterAdjusters.lastDayOfQuarter());
        return new TimeRange(
                start.atStartOfDay(),
                LocalDateTime.of(end, LocalTime.MAX),
                "本季度"
        );
    }
}