...
1. **密钥迁移策略** — RESOLVED: 保持API兼容性(per D-07)，无需数据迁移
   - 已知情况：现有使用DES加密的数据无法用AES解密
   - 不清楚：是否需要数据迁移？还是只影响未来数据？
   - 推荐：如果DES加密数据需要保留，实现双向兼容性（检测旧格式并迁移）
   - **决策:** 保持API向后兼容，DES方法标记@Deprecated但保留实现，新代码使用AES

2. **密钥存储安全性** — RESOLVED: 使用环境变量+配置文件双重支持(per D-01~D-03)，暂不引入jasypt
   - 已知情况：密钥将从配置注入，但配置本身需要保护
   - 不清楚：是否应该使用jasypt或类似工具加密配置中的密钥？
   - 推荐：建议使用环境变量或专用密钥管理服务
   - **决策:** 支持环境变量优先、配置文件备选的双重注入方式，暂不引入额外加密依赖
